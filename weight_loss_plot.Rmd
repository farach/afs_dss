...
# Load packages
pacman::p_load(tidyverse, ggforce, patchwork, ggtext, scico, ggimage)
...
weight <- readxl::read_excel("./garmin/Weight.xlsx") %>%
  janitor::clean_names() %>%
  mutate(
...
macro <- data.frame(
  date = as.Date(c("2020-05-23", "2020-06-27", "2020-07-25", "2020-10-31", "2020-11-14")),
...
# Make new table with distinct values to remove repition in graphic
df2 <- df1 %>%
  ...
  mutate(
    ...
    period_weight_pos = if_else(period %in% c("0", "1", "2", "3"), y_min - 5.5, y_max + 4.5)
  )
```
...
p <- df1 %>%
  ...
  geom_text(
    aes(
      x = date, y = period_weight_pos,
      label = paste0("Cal: ", scales::comma(total_cal), "\nP: ", p, 
                     "\nF: ", f, "\nC: ", c, "\nLosing weight too\nfast, nutrition\nadjustment")
    ),
    position = position_dodge(width = 0.9), size = 2.75,
...
(p <- p +
  ...
  ggtext::geom_textbox(
    data = tibble(x = as.Date("2020-09-15"), y = 177, label = "<span style='font-size:12pt;'>Shoulder Injury</span><br><br>From 08-22 through 09-19 I stopped tracking my training due to a shoulder injury that prevented me from working out. During this time I stopped tracking my diet as well but maintained a loose adherence to it."),
    aes(x, y, label = label),
...
```{r}
img2 <- here::here("img", "before_after.jpg")

(i <- ggplot() +
  # Add the before and after pic
  geom_image(aes(x = 0, y = 2.1, image = img2), size = 0.75) +
  # Add before label to before pic
  geom_text(aes(x = -1.25, y = 4, label = "Before")) +
  # Add after label to after pic
  geom_text(aes(x = 1.25, y = 4, label = "After")) +
  # Add summary of the plot
  geom_text(aes(
    x = 0, y = 9,
    label = str_wrap("During the 2020 pandy I obtained the services of a personal trainer. I had always wanted to get in shape and figured it was a great time to do so. I wasn't out in the tempting sea of fast food restaraunt and bars. I was at home where I could focus and remove temptations. This was not easy but not terribly difficult either. Key findings from the experience were to give your body time to adapt before making changes, track as much as possible to make adjustments easier, and be consistent.", 40)
  ),
  vjust = 1
  ) +
...
```{r}
bm <- readxl::read_excel("./garmin/body_measurements.xlsx") %>%
  janitor::clean_names()
...
bm <- bm %>%
  ...
  mutate(
    metric = factor(metric, 
                    levels = c("chest", "l_arm", "r_arm", "x2_above", "belly", 
                               "x2_below", "hips", "l_thigh", "r_thigh"), 
                    labels = c("Chest", "Left arm", "Right arm", '2" above belly',
...
(u <- bm %>%
  ...
  theme(
    legend.position = "none",
    plot.title = element_text(margin = margin(0, 0, 30, 0), hjust = 0, size = 12)
  ) +
  ...
  geom_mark_circle(aes(x = as.Date("2020-09-26"), y = 33.5, label = str_wrap("Right arm atrophy due to shoulder injury", 12), filter = metric == "Right arm"),
    color = "black",
